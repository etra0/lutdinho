...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let directory = path::PathBuf::from(matches.value_of("FOLDER").unwrap());
    ...
    for file in fs::read_dir(&directory)? {
        ...
        if !full_name.ends_with(".cube") { continue; }
    ...
    let fx_file = format!(include_str!("Template.fx"), lut_name, lut_names, png_name, tilesize_xy, tile_amount, lut_amount);
    println!("{}", fx_file);

    let generated_images: Vec<image::DynamicImage> = images
        .iter()
        .map(|x| {
            x.generate_image(None).unwrap()
        }).collect();
